...
import utilities.general as utils
...
if __name__ == "__main__":
    ...
    #----SKYFIELD------
    e_center = sf.Topos(latitude_degrees=0,
                  longitude_degrees=0,
                  elevation_m=-6378136.6)
    moon_astro = (earth+e_center).at(t).observe(moon)
    r_moon2 = moon_astro.position.km.transpose()
    mag_moon2 = np.linalg.norm(np.array(r_moon2), axis=1) #earth center to moon center
    print("\nSKYFIELD Moon X [km]:", np.array(r_moon2).transpose()[0])
    print("SKYFIELD Moon Y [km]:", np.array(r_moon2).transpose()[1])
    print("SKYFIELD Moon Z [km]:", np.array(r_moon2).transpose()[2])
    print("SKYFIELD MOON R [km]:", mag_moon2)
    ...
    rho_moon2 = moon_rv.position.km.transpose()
    rho_moon2_mag = np.linalg.norm(np.array(rho_moon2), axis=1)
    [el_moon,az_moon,range_moon] = moon_rv.apparent().altaz()
    ...
    print("\n      SKYFIELD Moon Azimuth [deg]*:", az_moon.degrees)
    print("    SKYFIELD Moon Elevation [deg]*:", el_moon.degrees)
    print("         SKYFIELD Moon Range [km]*:", range_moon.km)
    print("         RHO SKYFIELD Moon X [km]*:", np.array(rho_moon2).transpose()[0])
    print("         RHO SKYFIELD Moon Y [km]*:", np.array(rho_moon2).transpose()[1])
    print("         RHO SKYFIELD Moon Z [km]*:", np.array(rho_moon2).transpose()[2])
    print("     RHO SKYFIELD MOON Range [km]*:", rho_moon2_mag)
    ...
    theta_1 = np.arccos(np.divide(utils.R_moon, rho_moon_mag)) * utils.rad2deg
    theta_2 = np.arccos(np.divide(utils.R_moon, mag_ms)) * utils.rad2deg
    #theta_1 = np.arccos(np.divide(utils.R_moon, max(rho_moon2_mag))) *utils.rad2deg
    #theta_2 = np.arccos(np.divide(utils.R_moon, max(mag_ms))) * utils.rad2deg
    theta_12 = theta_1 + theta_2
    print('   theta_1 [deg]:', theta_1)
    print('   theta_2 [deg]:', theta_2)
    print('  theta_12 [deg]:', theta_12)

    theta = np.arccos(np.divide(np.diag(np.dot(rho_moon2, rho_ms.transpose())),rho_moon2_mag*mag_ms))* utils.rad2deg
    print('theta [deg]:', theta)

    print('\ni,theta,theta_1, theta_2,theta_12, LOS')
    for i, th in enumerate(theta):
        print (i, t[i].utc_iso(), lro_alt[i], th, theta_1[i], theta_2[i], theta_12[i], th <= theta_12[i])
    ...
    rho_moon2 = (earth+gs).at(t).observe(moon)
    [el_moon,az_moon,rho_moon] = rho_moon2.apparent().altaz()
    print(type(rho_moon2))
    print("           Moon Azimuth [deg]*:", np.array(df['MOON Azimuth [deg]']).transpose())
    print("         Moon Elevation [deg]*:", np.array(df['MOON Elevation [deg]']).transpose())
    print("              Moon Range [km]*:", np.array(df['MOON Range [km]']).transpose())
    sys.exit()
