...
# Comments:
# - Utilities for Astroquery data download from Horizons
#########################################
import sys
import os
import math
...
def query_ephemerides(cfg, target):
    #create observer location
    gs = {'lon': cfg['gs']['longitude'],
          'lat': cfg['gs']['latitude'],
          'elevation': cfg['gs']['altitude_m']/1000.0}
    #create query object
    id=cfg['horizons']['targets'][target]
    obj = Horizons(id       = id,
                   location = gs,
...
def generate_vectors_file_name(cfg, target):
    data_path = '/'.join([cfg['cwd'], cfg['data_path']])
    if not os.path.exists(data_path):
        print('Data path doesn\'t exist, creating...: {:s}'.format(data_path))
        os.makedirs(data_path)
